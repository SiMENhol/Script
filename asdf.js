...
function checkSpinningWheelOutcome() {
  ...
  if (spinningWheelCountdown.textContent === '0,00') {
    ...
    // Log the countdown time until the spinning wheel starts rolling
    console.log('Countdown: ' + spinningWheelCountdown.textContent);

    // Check the outcome of previous rolls only once during countdown
    if (!localStorage.getItem('previousRollChecked')) {
      ...
      localStorage.setItem('previousRollChecked', true);
    }
  }
